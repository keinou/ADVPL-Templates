...
Static Function fCopyFile()
	Local _nCpy := Aviso( "Copiar arquivo", "Selecione qual opÁ„o de copia deseja utilizar" ,{"Local -> Servidor", "Serivodr -> Local", "cancelar"} , 3 )
...
Static Function fTcSqlExec()
	Local _cSQL := ""
	Local _cAviso := "Confirma a execuÁ„o do sql (digite abaixo)? essa opÁ„o executa insert, update, delete, qualquer coisa, por sua conta em risco"
...
Static Function fHelp()
	...
	_cMsgHelp += CRLF
	_cMsgHelp += "Toda vez que o sql È executado um backup do sql È salvo no diretorio \sql_auto_save\" + CRLF
...
If !_lP11
	Alert("Essa vers„o do protheus n„o suporta a geraÁ„o de excel xml.")
	Return nil
EndIf
...
FunÁ„o que abre a tela padr„o do Windows Explorer para escolher um arquivo
Substitui a cGetFile
@author Atilio
@since 03/08/2017
@version undefined
@param cMascara, characters, M·scara para escolha de arquivos (DescriÁ„o *.extens„o | *.extens„o)
@type function
...
Static Function zChooseFile(cMascara)
    ...
    //Monta o comando para abrir a tela de seleÁ„o do windows
    ...
    //Executando o comando atravÈs do .bat
