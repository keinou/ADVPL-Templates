...
Static Function fMainFun()
	...
	oBImpCSV := TMenuItem():New(oMainWnd,"Import csv"     ,,,,{||;
		FWMsgRun( /* Obj_oMainWnd */ , {|oSay| U_fImpCSV() } , "Aguarde" , "Importando csv..." ) } ,,,,,,,,,.T.)
...
User FUNCTION Aviso(cCaption,cMensagem,aBotoes,_lHabDigt)
	...
	For nx:=1 to Len(aBotoes)
		...
		TButton():New(_aObj[2,1], ny ,cMsgButton, oDlgAviso,bAction,_nLarg,15,,_oFCons16,.F.,.T.,.F.,,.F.,,,.F.)
		ny += _nLarg + 3
	Next nx
